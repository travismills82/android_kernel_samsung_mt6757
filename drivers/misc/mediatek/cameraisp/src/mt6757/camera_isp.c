...
#define CAMSYS_REG_SW_RST               (ISP_CAMSYS_CONFIG_BASE + 0xC)
			...
			if (!bBlockBigData) {
				...
				if (mMclk1User == 1) {
					...
					if (hw_param) {
						...
						LOG_ERR("[BigData] rear mipi error occured (cnt:%d)\n", hw_param->mipi_sensor_err_cnt);
					}
				} else if (mMclk1User == 2) {
					is_sec_get_rear2_hw_param(&hw_param);
					if (hw_param) {
						hw_param->mipi_sensor_err_cnt++;
						LOG_ERR("[BigData] mono camera mipi error occured (cnt:%d)\n", hw_param->mipi_sensor_err_cnt);
					}				
				} else if (mMclk2User) {
					is_sec_get_front_hw_param(&hw_param);
					if (hw_param) {
						hw_param->mipi_sensor_err_cnt++;
						LOG_ERR("[BigData] front mipi error occured (cnt:%d)\n", hw_param->mipi_sensor_err_cnt);
					}
				} else {
					LOG_ERR("[BigData] mipi error occured But no camera error occured !!!!!"
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case ISP_DUMP_REG:
		...
		Ret = ISP_DumpReg();
		break;
	case ISP_RESET_CAMSYS:
		/* Reset uni, cama, camb, cam* ..., their reg will be default value.
		*  at reset timing, DO NOT read any reg of them
		*/
		LOG_INF("reset cam.\n");
		ISP_WR32(CAMSYS_REG_SW_RST,0x30);
		ISP_WR32(CAMSYS_REG_SW_RST,0x00);
		mdelay(1);
		break;
	case ISP_DEBUG_FLAG:
