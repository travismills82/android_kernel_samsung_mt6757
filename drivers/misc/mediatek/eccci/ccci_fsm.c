...
static int ccci_fsm_main(void *data)
{
	...
	while (1) {
		...
		case CCCI_COMMAND_STOP:
			...
			spin_lock_irqsave(&modem_meta_reset_wq.lock, flags);
			modem_meta_reset_flag = 1;
			wake_up_all_locked(&modem_meta_reset_wq);
			spin_unlock_irqrestore(&modem_meta_reset_wq.lock, flags);
			CCCI_ERROR_LOG(md->index, CHAR, "ccci_fsm_main wakeup called\n");
			break;
