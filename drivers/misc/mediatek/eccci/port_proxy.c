...
wait_queue_head_t modem_meta_reset_wq;
int modem_meta_reset_flag;
...
static inline void port_struct_init(struct ccci_port *port, struct port_proxy *port_p)
{
	...
	init_waitqueue_head(&modem_meta_reset_wq);
...
#ifdef CONFIG_MTK_ECCCI_C2K
		if (ccci_md_get_state_by_id(md_id) == EXCEPTION) {
			while (count < 10) {
				if (ccci_md_get_state_by_id(MD_SYS1) == EXCEPTION) {
					CCCI_NORMAL_LOG(md_id, CHAR, "md1 exception state is checked\n");
					break;
				}
				CCCI_ERROR_LOG(md_id, CHAR, "wait md1 exception... state == %d\n", \
						ccci_md_get_state_by_id(MD_SYS1));
				count++;
				msleep(500);
			}
		}
...
#endif
#endif
		spin_lock_irq(&modem_meta_reset_wq.lock);
		ret = wait_event_interruptible_locked_irq(modem_meta_reset_wq, modem_meta_reset_flag);
		modem_meta_reset_flag = 0;
		spin_unlock_irq(&modem_meta_reset_wq.lock);
		if (ret == -ERESTARTSYS)
			ret = -EINTR;
		break;
	...
	case CCCI_IOC_FORCE_MD_OFF:
		{
			...
			if (md_id == MD_SYS1) {
				...
				//exec_ccci_kern_func_by_md_id(MD_SYS3, ID_ENTER_FLIGHT_MODE, NULL, 0);
				ret = port_proxy_send_msg_to_user(md3_power_off_proxy_p, CCCI_MONITOR_CH, CCCI_MD_MSG_FLIGHT_STOP_REQUEST, 0);
				msleep(500);
				#endif
			...
			CCCI_ERROR_LOG(md_id, CHAR, "CCCI_IOC_FORCE_MD_OFF stop MD request ioctl called by %s\n", current->comm);
			ret = port_proxy_send_msg_to_user(proxy_p, CCCI_MONITOR_CH, CCCI_MD_MSG_FORCE_STOP_REQUEST, 0);
		}
		break;
