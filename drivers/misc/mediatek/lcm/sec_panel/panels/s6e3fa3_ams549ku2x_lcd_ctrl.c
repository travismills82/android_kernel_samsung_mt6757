...
struct abd_protect {
	...
	ktime_t det_timestamp[10];
	ktime_t pcd_timestamp[10];
	unsigned int det_levelstamp[10];
	unsigned int pcd_levelstamp[10];
	int det_timestamp_cnt;
	int pcd_timestamp_cnt;
...
struct lcd_info {
	...
	unsigned int			state;
	unsigned int			enable;
	unsigned int			shutdown; //if set, set trying to shutdown.
...
static void panel_abd_enable_interrupt(struct lcd_info *lcd, bool enable)
{
	...
	if (enable) {
		...
		if (abd->pcd_irq) {
			dev_info(&lcd->ld->dev, "%s: %d, pcd(%d) level: %d\n", __func__, enable, abd->pcd_irq, gpio_get_value(abd->pcd_gpio));
			panel_abd_clear_pending_bit(abd->pcd_irq);
			enable_irq(abd->pcd_irq);
		}
		if (abd->err_irq) {
			dev_info(&lcd->ld->dev, "%s: %d, err(%d) level: %d\n", __func__, enable, abd->err_irq, gpio_get_value(abd->err_gpio));
			panel_abd_clear_pending_bit(abd->err_irq);
			enable_irq(abd->err_irq);
		}
		if (abd->det_irq) {
			dev_info(&lcd->ld->dev, "%s: %d, det(%d) level: %d\n", __func__, enable, abd->det_irq, gpio_get_value(abd->det_gpio));
			panel_abd_clear_pending_bit(abd->det_irq);
...
irqreturn_t panel_abd_pcd_handler(int irq, void *dev_id)
{
	...
	dev_info(&lcd->ld->dev, "%s: level: %d, state: %d\n",  __func__, level, lcd->state);

	abd->pcd_timestamp[(abd->pcd_timestamp_cnt)%10] = ktime_get();
	abd->pcd_levelstamp[(abd->pcd_timestamp_cnt++)%10] = level;
	return IRQ_HANDLED;
}

irqreturn_t panel_abd_err_handler(int irq, void *dev_id)
{
	struct lcd_info *lcd= (struct lcd_info *)dev_id;
	struct abd_protect *abd = &lcd->abd;
	int level = gpio_get_value(abd->pcd_gpio);
...
irqreturn_t panel_abd_det_handler(int irq, void *dev_id)
{
	...
	abd->det_timestamp[(abd->det_timestamp_cnt)%10] = ktime_get();
	abd->det_levelstamp[(abd->det_timestamp_cnt++)%10] = level;
...
static int debug_event_show(struct seq_file *s, void *unused)
{
	struct lcd_info *lcd = s->private;
	struct abd_protect *abd = &lcd->abd;
	struct timeval tv;
	int i = 0;

	seq_printf(s, "=========== LCD DEBUG ===========\n");
	seq_printf(s, "[LCD Connected]: %d\n", lcd->connected);
	seq_printf(s, "[ABD DET interrupts]\n");
	seq_printf(s, "\tTotal Count: %d\n", abd->det_timestamp_cnt);
	for (i = 0; i < 10; i++) {
		tv = ns_to_timeval(abd->det_timestamp[i].tv64);
		seq_printf(s, "\tTime: %lu.%06lu level: %d \n", (unsigned long)tv.tv_sec, tv.tv_usec, abd->det_levelstamp[i]);
	}
	seq_printf(s, "[ABD PCD interrupts]\n");
	seq_printf(s, "\tTotal Count: %d\n", abd->pcd_timestamp_cnt);
	for (i = 0; i < 10; i++) {
		tv = ns_to_timeval(abd->pcd_timestamp[i].tv64);
		seq_printf(s, "\tTime: %lu.%06lu level: %d \n", (unsigned long)tv.tv_sec, tv.tv_usec, abd->pcd_levelstamp[i]);
	}
...
static int s6e3fa3_abd_register_function(struct lcd_info *lcd)
{
	int ret = 0;
	struct abd_protect *abd = &lcd->abd;
	...
	abd->pcd_gpio = of_get_named_gpio_flags(np, "gpio_pcd", 0, &flags);
	if (gpio_is_valid(abd->pcd_gpio)) {
		dev_info(&lcd->ld->dev, "%s: found gpio_pcd(%d) success\n", __func__, abd->pcd_gpio);
	...
	if (abd->pcd_irq) {
		...
		if (devm_request_irq(dev, abd->pcd_irq, panel_abd_pcd_handler,
				pcd_irqf_type, "pcd-irq", lcd)) {
			dev_err(&lcd->ld->dev, "%s: failed to request irq for pcd\n", __func__);
			abd->pcd_irq = 0;
			ret--;
		}
	}
	if (abd->err_irq) {
		...
		if (devm_request_irq(dev, abd->err_irq, panel_abd_err_handler,
				err_irqf_type, "err-irq", lcd)) {
			dev_err(&lcd->ld->dev, "%s: failed to request irq for err\n", __func__);
			abd->err_irq = 0;
			ret--;
		}
	}
	if (abd->det_irq) {
		...
		if (devm_request_irq(dev, abd->det_irq, panel_abd_det_handler,
				det_irqf_type, "det-irq", lcd)) {
			dev_err(&lcd->ld->dev, "%s: failed to request irq for det\n", __func__);
			abd->det_irq = 0;
			ret--;
		}
	}
			...
			unsigned long event, void *data)
{
	...
	if (event == FB_EARLY_EVENT_BLANK && fb_blank == FB_BLANK_POWERDOWN) {
		mutex_lock(&lcd->lock);
		panel_abd_enable_interrupt(lcd, false);
		lcd->enable = 0;
		mutex_unlock(&lcd->lock);
		...
		if (lcd->shutdown) {
			...
			lcd->enable = 1;
			panel_abd_enable_interrupt(lcd, true);
		}
		mutex_unlock(&lcd->lock);
		dsi_panel_set_brightness(lcd, 1);
	}
...
static int dsi_panel_probe(struct platform_device *pdev)
{
	...
	if (lcd->connected)
		s6e3fa3_abd_register_function(lcd);
