...
static atomic_t idlemgr_wrot_status = ATOMIC_INIT(0);
/* dvfs */
...
int get_idlemgr_wrot_status(void)
{
	return atomic_read(&idlemgr_wrot_status);
}

void enter_share_sram(enum CMDQ_EVENT_ENUM resourceEvent)
{
	if (resourceEvent == CMDQ_SYNC_RESOURCE_WROT1)
		atomic_inc(&idlemgr_wrot_status);
...
{
	if (resourceEvent == CMDQ_SYNC_RESOURCE_WROT1)
		atomic_dec(&idlemgr_wrot_status);
