...
void _cmdq_start_trigger_loop(void)
{
	...
	if (!primary_display_is_video_mode()) {
		...
		while (!cmdqCoreGetEvent(CMDQ_SYNC_TOKEN_STREAM_EOF)) {
			...
			DISPWARN("set CMDQ_SYNC_TOKEN_STREAM_EOF = %d\n",
...
int primary_display_suspend(void)
{
	...
	pr_info("%25s: %4d: %d, %d\n", __func__, __LINE__, get_idlemgr_wrot_status(), primary_display_get_power_mode_nolock());
	return ret;
}
...
int primary_display_resume(void)
{
	...
	if (pgc->state == DISP_ALIVE) {
		...
		/* SEC: _primary_path_lock(__func__); */
		/* SEC: goto done; */
		pr_info("%25s: %4d: %d, %d, %d\n", __func__, __LINE__, get_idlemgr_wrot_status(), primary_display_get_power_mode_nolock(), skip_update);
		return ret;
	}
