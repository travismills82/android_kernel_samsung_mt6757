...
static void isa1000_pwm_config(struct isa1000_ddata *ddata, int duty)
{
	...
	if (duty > max_duty)
		...
		duty = min_duty;
	pwm_config(ddata->pwm, duty, ddata->pdata->period);
}
...
static struct isa1000_pdata *isa1000_get_devtree_pdata(struct device *dev)
{
	...
	pr_info("[VIB] max_timeout = %d\n", pdata->max_timeout);
	pr_info("[VIB] ldo_type = %d\n", pdata->ldo_type);
	pr_info("[VIB] duty = %d\n", pdata->duty);
	pr_info("[VIB] period = %d\n", pdata->period);
	pr_info("[VIB] pwm_id = %d\n", pdata->pwm_id);
	pr_info("[VIB] gpio_en = %d\n", pdata->gpio_en);
...
#if defined(CONFIG_OF)
static const struct of_device_id isa1000_dt_ids[] = {
	{ .compatible = "isa1000" },
	{ }
};
