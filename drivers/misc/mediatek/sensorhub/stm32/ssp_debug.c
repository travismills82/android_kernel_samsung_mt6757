...
void ssp_dump_task(struct work_struct *work)
{
	...
	/* buffer aligned*/
	staitc char buffer[DATA_PACKET_SIZE] __aligned(4);
	memset(buffer, 0, sizeof(char) * DATA_PACKET_SIZE);
...
void sync_sensor_state(struct ssp_data *data)
{
	...
	for (uSensorCnt = 0; uSensorCnt < SSP_SENSOR_TYPE_MAX; uSensorCnt++) {
	...
	if (data->is_proxraw_enabled == true) {
		...
		memcpy(&buf[0], &dMsDelay, 4);
		send_instruction(data, ADD_SENSOR, SSP_SENSOR_TYPE_PROXIMITY_RAW, buf, 4);
	}

	set_light_coef(data);
	set_proximity_threshold(data);
	report_sensor_data(data, SSP_SENSOR_TYPE_PROXIMITY, &data->buf[SSP_SENSOR_TYPE_PROXIMITY]);
...
static void print_sensordata(struct ssp_data *data, unsigned int uSensor)
{
	...
	case SSP_SENSOR_TYPE_INTERRUPT_GYRO:
	...
	case SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD:
		ssp_info("%u : %d, %d, %d, %d (%ums)", uSensor,
			...
			get_msdelay(data->delay[uSensor]));
		break;
	case SSP_SENSOR_TYPE_MAGNETIC_FIELD_UNCALIBRATED:
	...
	case SSP_SENSOR_TYPE_PROXIMITY:
		ssp_info("%u : %d, %d (%ums)", uSensor,
			...
			get_msdelay(data->delay[uSensor]));
		break;
	case SSP_SENSOR_TYPE_STEP_DETECTOR:
		ssp_info("%u : %u (%ums, %dms)", uSensor,
			data->buf[uSensor].step_det,
			get_msdelay(data->delay[uSensor]),
			data->batch_max_latency[uSensor]);
		break;
	case SSP_SENSOR_TYPE_GAME_ROTATION_VECTOR:
	case SSP_SENSOR_TYPE_ROTATION_VECTOR:
		ssp_info("%u : %d, %d, %d, %d, %d (%ums, %dms)", uSensor,
			...
			data->batch_max_latency[uSensor]);
		break;
	case SSP_SENSOR_TYPE_SIGNIFICANT_MOTION:
		ssp_info("%u : %u(%ums)", uSensor,
			data->buf[uSensor].sig_motion,
			get_msdelay(data->delay[uSensor]));
		break;
	case SSP_SENSOR_TYPE_GYROSCOPE_UNCALIBRATED:
		ssp_info("%u : %d, %d, %d, %d, %d, %d (%ums)", uSensor,
			...
			get_msdelay(data->delay[uSensor]));
		break;
	case SSP_SENSOR_TYPE_STEP_COUNTER:
		ssp_info("%u : %u(%ums)", uSensor,
...
bool check_wait_event(struct ssp_data *data)
{
	...
	int check_sensors[2] = {SSP_SENSOR_TYPE_ACCELEROMETER, SSP_SENSOR_TYPE_LIGHT};
	int i, sensor;
	bool res = false;
...
static void debug_work_func(struct work_struct *work)
{
	...
	if (((atomic64_read(&data->aSensorEnable) & (1 << SSP_SENSOR_TYPE_ACCELEROMETER))
		&& (data->batch_max_latency[SSP_SENSOR_TYPE_ACCELEROMETER] == 0)
