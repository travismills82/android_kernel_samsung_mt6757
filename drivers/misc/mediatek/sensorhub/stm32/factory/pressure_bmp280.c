	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = sscanf(buf, "%9d", &data->buf[SSP_SENSOR_TYPE_PRESSURE].pressure_sealevel);

	if (data->buf[SSP_SENSOR_TYPE_PRESSURE].pressure_sealevel == 0) {
		pr_info("%s, our->temperature = 0\n", __func__);
		data->buf[SSP_SENSOR_TYPE_PRESSURE].pressure_sealevel = -1;
	}

	pr_info("[SSP] %s sea_level_pressure = %d\n",
		__func__, data->buf[SSP_SENSOR_TYPE_PRESSURE].pressure_sealevel);
	return size;
}
...
int pressure_open_calibration(struct ssp_data *data)
{
	...
	iErr = kstrtoint(chBuf, 10, &data->buf[SSP_SENSOR_TYPE_PRESSURE].pressure_cal);
	if (iErr < 0) {
	...
	if (data->buf[SSP_SENSOR_TYPE_PRESSURE].pressure_cal < PR_ABS_MIN
		|| data->buf[SSP_SENSOR_TYPE_PRESSURE].pressure_cal > PR_ABS_MAX)
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	data->buf[SSP_SENSOR_TYPE_PRESSURE].pressure_cal = (s32)iPressureCal;
	...
	struct device_attribute *attr, char *buf)
{
	...
	s32 temp = 0;
	temp = (s32) (data->buf[SSP_SENSOR_TYPE_PRESSURE].temperature);
	temperature = (4250) + ((temp / (120 * 4))*100);
	/*(42.5f) + (temperature/(120 * 4));*/
...
{
	sensors_register(&data->devices[SSP_SENSOR_TYPE_PRESSURE], data, pressure_attrs,
		"barometer_sensor");
}

void remove_pressure_factorytest(struct ssp_data *data)
{
	sensors_unregister(data->devices[SSP_SENSOR_TYPE_PRESSURE], pressure_attrs);
}
