...
int set_gyro_cal(struct ssp_data *data)
{
	...
	/* buffer aligned*/
	static char gyro_cal[6] __aligned(4);

	memset(gyro_cal, 0, sizeof(char) * 6);
	if (!(data->uSensorState & (1 << SSP_SENSOR_TYPE_GYROSCOPE))) {
		pr_info("[SSP]: %s - Skip this function!, gyro sensor is not connected(0x%llx)\n",
			__func__, data->uSensorState);
		return ret;
	}

	gyro_cal[0] = ((char)(data->gyrocal.x >> 8)) & 0xff;
	gyro_cal[1] = (char)data->gyrocal.x;
	gyro_cal[2] = ((char)(data->gyrocal.y >> 8)) & 0xff;
	gyro_cal[3] = (char)data->gyrocal.y;
	gyro_cal[4] = ((char)(data->gyrocal.z >> 8)) & 0xff;
	gyro_cal[5] = (char)data->gyrocal.z;
...
short do_gyro_get_temp(struct ssp_data *data)
{
	...
	/* buffer aligned*/
	static char temp[2] __aligned(4);
...
static ssize_t k6ds3tr_gyro_selftest(char *buf, struct ssp_data *data)
{
	...
	/* buffer aligned*/
	static char chTempBuf[36] __aligned(4);
	...
	/* buffer aligned*/
	memset(chTempBuf, 0, sizeof(char) * 36);
	...
	for (j = 0; j < 3; j++) {
		if (unlikely(rms[j] / total_count > DEF_RMS_THRESH)) {
			pr_err("[SSP] %s-Gyro rms (%ld) exceeded threshold(threshold = %d LSB)\n",
				a_name[j], rms[j] / total_count, DEF_RMS_THRESH);
			ret_val |= 1 << (7 + j);
		}
	}
...
exit:
	ssp_infof("%d,%d.%03d,%d.%03d,%d.%03d,%d.%03d,%d.%03d,%d.%03d,%d.%d,%d.%d,%d.%d,%d,%d,%d\n",
		ret_val,
...
ssize_t bmi168_gyro_selftest(char *buf, struct ssp_data *data)
{
	...
	/* buffer aligned*/
	static char chTempBuf[19] __aligned(4);
	...
	/* buffer aligned*/
	memset(chTempBuf, 0, sizeof(char) * 19);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/* buffer aligned*/
	static char chTempBuf __aligned(4);
	chTempBuf = 0;
	...
	data->buf[SSP_SENSOR_TYPE_GYROSCOPE].gyro_dps = (unsigned int)new_dps;
	pr_err("[SSP]: %s - %u dps stored\n", __func__,
			data->buf[SSP_SENSOR_TYPE_GYROSCOPE].gyro_dps);
exit:
	return count;
}
...
{
	sensors_register(&data->devices[SSP_SENSOR_TYPE_GYROSCOPE], data, gyro_attrs,
			data->info[SSP_SENSOR_TYPE_GYROSCOPE].name);
}

void remove_gyro_factorytest(struct ssp_data *data)
{
	sensors_unregister(data->devices[SSP_SENSOR_TYPE_GYROSCOPE], gyro_attrs);
}
