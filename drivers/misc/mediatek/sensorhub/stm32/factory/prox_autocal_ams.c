...
static u16 get_proximity_rawdata(struct ssp_data *data)
{
	...
	if (data->is_proxraw_enabled == false) {
		send_instruction(data, ADD_SENSOR, SSP_SENSOR_TYPE_PROXIMITY_RAW, chTempbuf, 4);
		msleep(200);
		uRowdata = data->buf[SSP_SENSOR_TYPE_PROXIMITY_RAW].prox_raw[0];
		send_instruction(data, REMOVE_SENSOR, SSP_SENSOR_TYPE_PROXIMITY_RAW,
			chTempbuf, 4);
	} else {
		uRowdata = data->buf[SSP_SENSOR_TYPE_PROXIMITY_RAW].prox_raw[0];
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d\n",
		data->buf[SSP_SENSOR_TYPE_PROXIMITY_RAW].prox_raw[1],
		data->buf[SSP_SENSOR_TYPE_PROXIMITY_RAW].prox_raw[2],
		data->buf[SSP_SENSOR_TYPE_PROXIMITY_RAW].prox_raw[3]);
}
...
{
	sensors_register(&data->devices[SSP_SENSOR_TYPE_PROXIMITY], data,
		prox_attrs, "proximity_sensor");
}

void remove_prox_factorytest(struct ssp_data *data)
{
	sensors_unregister(data->devices[SSP_SENSOR_TYPE_PROXIMITY], prox_attrs);
}
