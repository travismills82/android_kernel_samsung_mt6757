...
{
	if ((data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].x == 0) &&
...
{
	if ((data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].x == 0) &&
		(data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].y == 0) &&
		(data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].z == 0))
		return FAIL;
	else if ((data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].x > 16383) ||
		(data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].x < -16383) ||
		(data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].y > 16383) ||
		(data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].y < -16383) ||
		(data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].z > 16383) ||
		(data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].z < -16383))
		return FAIL;
	else
		return SUCCESS;
	...
	struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d %d %d %d %d %d %d %d %d\n",
				...
				data->static_matrix[2], data->static_matrix[3],
				data->static_matrix[4], data->static_matrix[5],
				data->static_matrix[6], data->static_matrix[7],
