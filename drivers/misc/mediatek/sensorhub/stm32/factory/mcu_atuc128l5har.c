	...
	struct device_attribute *attr, char *buf)
{
	...
	struct ssp_data *data = dev_get_drvdata(dev);
	struct sensor_value fsb[SSP_SENSOR_TYPE_MAX];
	u16 chLength = 0;

	memcpy(&chLength, buffer, 2);
	memset(fsb, 0, sizeof(struct sensor_value) * SSP_SENSOR_TYPE_MAX);

	for (iDataIdx = 2; iDataIdx < chLength + 2;) {
		iSensorData = (int)buffer[iDataIdx++];

		if ((iSensorData < 0) ||
			(iSensorData >= (SSP_SENSOR_TYPE_MAX - 1))) {
	...
	fsb[SSP_SENSOR_TYPE_PRESSURE].pressure
			-= data->buf[SSP_SENSOR_TYPE_PRESSURE].pressure_cal;
...
exit:
	ssp_infof("Result\n"
		...
		"[SSP]: temp %d,%d,%d\n",
		fsb[SSP_SENSOR_TYPE_ACCELEROMETER].x, fsb[SSP_SENSOR_TYPE_ACCELEROMETER].y,
		fsb[SSP_SENSOR_TYPE_ACCELEROMETER].z, fsb[SSP_SENSOR_TYPE_GYROSCOPE].x,
		fsb[SSP_SENSOR_TYPE_GYROSCOPE].y, fsb[SSP_SENSOR_TYPE_GYROSCOPE].z,
		fsb[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].cal_x, fsb[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].cal_y,
		fsb[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].cal_z, fsb[SSP_SENSOR_TYPE_PRESSURE].pressure,
		fsb[SSP_SENSOR_TYPE_PRESSURE].temperature,
		fsb[SSP_SENSOR_TYPE_PROXIMITY].prox, fsb[SSP_SENSOR_TYPE_PROXIMITY].prox_ex,
		fsb[SSP_SENSOR_TYPE_LIGHT].r, fsb[SSP_SENSOR_TYPE_LIGHT].g, fsb[SSP_SENSOR_TYPE_LIGHT].b,
		fsb[SSP_SENSOR_TYPE_LIGHT].w,
		fsb[SSP_SENSOR_TYPE_LIGHT].a_time, fsb[SSP_SENSOR_TYPE_LIGHT].a_gain,
		fsb[SSP_SENSOR_TYPE_TEMPERATURE].x, fsb[SSP_SENSOR_TYPE_TEMPERATURE].y,
		fsb[SSP_SENSOR_TYPE_TEMPERATURE].z);
	...
	return sprintf(buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%u,"
		...
		fsb[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].cal_x, fsb[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].cal_y,
		fsb[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].cal_z, fsb[SSP_SENSOR_TYPE_PRESSURE].pressure,
		fsb[SSP_SENSOR_TYPE_PRESSURE].temperature, fsb[SSP_SENSOR_TYPE_PROXIMITY].prox_ex,
		fsb[SSP_SENSOR_TYPE_LIGHT].r, fsb[SSP_SENSOR_TYPE_LIGHT].g, fsb[SSP_SENSOR_TYPE_LIGHT].b,
		fsb[SSP_SENSOR_TYPE_LIGHT].w,
		fsb[SSP_SENSOR_TYPE_LIGHT].a_time, fsb[SSP_SENSOR_TYPE_LIGHT].a_gain,
