...
int set_accel_cal(struct ssp_data *data)
{
	...
	/* buffer aligned*/
	static char accel_cal[6] __aligned(4);

	memset(accel_cal, 0, sizeof(char) * 6);
	if (!(data->uSensorState & (1 << SSP_SENSOR_TYPE_ACCELEROMETER))) {
		pr_info("[SSP]: %s - Skip this function!, accel sensor is not connected(0x%llx)\n",
			__func__, data->uSensorState);
		return ret;
	}
	accel_cal[0] = ((char)(data->accelcal.x >> 8)) & 0xff;
	accel_cal[1] = (char)data->accelcal.x;
	accel_cal[2] = ((char)(data->accelcal.y >> 8)) & 0xff;
	accel_cal[3] = (char)data->accelcal.y;
	accel_cal[4] = ((char)(data->accelcal.z >> 8)) & 0xff;
	accel_cal[5] = (char)data->accelcal.z;

	msg = kzalloc(sizeof(*msg), GFP_KERNEL);
	msg->cmd = MSG2SSP_AP_MCU_SET_ACCEL_CAL;
	msg->length = 6;
	msg->options = AP2HUB_WRITE;
	msg->buffer = accel_cal;
	msg->free_buffer = 0;
...
{
	u8 buf[9] = { 0, };
	s32 dMsDelay = get_msdelay(data->delay[SSP_SENSOR_TYPE_ACCELEROMETER]);

	memcpy(&buf[0], &dMsDelay, 4);

	if (atomic64_read(&data->aSensorEnable) & (1 << SSP_SENSOR_TYPE_ACCELEROMETER)) {
		if (get_msdelay(data->delay[SSP_SENSOR_TYPE_ACCELEROMETER]) != 10) {
			send_instruction(data, CHANGE_DELAY,
				SSP_SENSOR_TYPE_ACCELEROMETER, buf, 9);
			return SUCCESS;
		}
	} else {
...
static void disable_accel_for_cal(struct ssp_data *data, int delay)
{
	...
	if (atomic64_read(&data->aSensorEnable) & (1 << SSP_SENSOR_TYPE_ACCELEROMETER)) {
		...
		send_instruction(data, REMOVE_SENSOR,
			SSP_SENSOR_TYPE_ACCELEROMETER, buf, 4);
	}
}
...
static int accel_do_calibrate(struct ssp_data *data, int enable)
{
	...
	if (enable) {
		...
		for (count = 0; count < CALIBRATION_DATA_AMOUNT; count++) {
			iSum[0] += data->buf[SSP_SENSOR_TYPE_ACCELEROMETER].x;
			iSum[1] += data->buf[SSP_SENSOR_TYPE_ACCELEROMETER].y;
			iSum[2] += data->buf[SSP_SENSOR_TYPE_ACCELEROMETER].z;
			mdelay(10);
		}
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d\n",
		data->buf[SSP_SENSOR_TYPE_ACCELEROMETER].x,
		data->buf[SSP_SENSOR_TYPE_ACCELEROMETER].y,
		data->buf[SSP_SENSOR_TYPE_ACCELEROMETER].z);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* buffer aligned*/
	static char temp __aligned(4) = 1;
	temp = 1;
...
static ssize_t mpu6500_accel_selftest(char *buf, struct ssp_data *data)
{
	...
	/* buffer aligned*/
	static char temp[8] __aligned(4);

	temp[0] = 2;
	memset(&temp[1], 0, sizeof(char) * 7);
...
static ssize_t accel_selftest(char *buf, struct ssp_data *data)
{
	...
	/* buffer aligned*/
	static char temp[12] __aligned(4);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* buffer aligned*/
	static char buffer[1] __aligned(4);
...
{
	sensors_register(&data->devices[SSP_SENSOR_TYPE_ACCELEROMETER], data, acc_attrs,
			data->info[SSP_SENSOR_TYPE_ACCELEROMETER].name);
}

void remove_accel_factorytest(struct ssp_data *data)
{
	sensors_unregister(data->devices[SSP_SENSOR_TYPE_ACCELEROMETER], acc_attrs);
}
