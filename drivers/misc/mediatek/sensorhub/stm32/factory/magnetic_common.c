		...
		struct device_attribute *attr, char *strbuf)
{
	...
	/* buffer aligned */
	static char chTempBuf[22] __aligned(4);

	memset(chTempBuf, 0, sizeof(char) * 22);
	...
	struct device_attribute *attr, char *buf)
{
	...
	/* buffer aligned */
	static char bufSelftset[22] __aligned(4);

	memset(bufSelftset, 0, sizeof(char) * 22);
...
Retry_selftest:
	...
	data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].x = 0;
	data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].y = 0;
	data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].z = 0;

	if (!(atomic64_read(&data->aSensorEnable) & (1 << SSP_SENSOR_TYPE_GEOMAGNETIC_POWER)))
		send_instruction(data, ADD_SENSOR, SSP_SENSOR_TYPE_GEOMAGNETIC_POWER,
			bufAdc, 4);

	do {
		msleep(60);
		if (check_data_spec(data, SSP_SENSOR_TYPE_GEOMAGNETIC_POWER) == SUCCESS)
			break;
	...
	iADC_X = data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].x;
	iADC_Y = data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].y;
	iADC_Z = data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].z;

	if (!(atomic64_read(&data->aSensorEnable) & (1 << SSP_SENSOR_TYPE_GEOMAGNETIC_POWER)))
		send_instruction(data, REMOVE_SENSOR, SSP_SENSOR_TYPE_GEOMAGNETIC_POWER,
			bufAdc, 4);
...
static int set_pdc_matrix(struct ssp_data *data)
{
	...
	/* buffer aligned */
	static char matrix[PDC_SIZE] __aligned(4);

	memset(matrix, 0, sizeof(char) * PDC_SIZE);
...
int get_fuserom_data(struct ssp_data *data)
{
	...
	/* buffer aligned */
	static char buffer[3] __aligned(4);
	...
	struct device_attribute *attr, char *buf)
{
	...
	/* buffer aligned */
	static char buffer[21] __aligned(4);
		...
		struct device_attribute *attr, char *buf)
{
	...
	/* buffer aligned */
	static char chTempBuf[24] __aligned(4);
	...
	/* buffer aligned */
	memset(chTempBuf, 0, sizeof(char) * 24);
...
int set_static_matrix(struct ssp_data *data)
{
	...
	/* buffer aligned */
	static char temp_matrix[18] __aligned(4);

	memset(temp_matrix, 0, sizeof(char) * 18);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (data->mag_type == AKM_ID) {
		return sprintf(buf,
			...
			data->pdc_matrix[12], data->pdc_matrix[13], data->pdc_matrix[14],
			data->pdc_matrix[15], data->pdc_matrix[16], data->pdc_matrix[17],
			data->pdc_matrix[18], data->pdc_matrix[19],	data->pdc_matrix[20],
			data->pdc_matrix[21], data->pdc_matrix[22], data->pdc_matrix[23],
			data->pdc_matrix[24], data->pdc_matrix[25], data->pdc_matrix[26]);
	} else {
		return sprintf(buf,
			"%d %d %d %d %d %d %d %d %d\n", data->static_matrix[0], data->static_matrix[1],
			data->static_matrix[2], data->static_matrix[3], data->static_matrix[4],
			data->static_matrix[5], data->static_matrix[6], data->static_matrix[7],
			data->static_matrix[8]);
	}
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("[SSP] %s - %d,%d,%d\n", __func__,
		data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].x,
		data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].y,
		data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].z);

	if (data->bGeomagneticRawEnabled == false) {
		data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].x = -1;
		data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].y = -1;
		data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_POWER].z = -1;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].x = 0;
	data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].y = 0;
	data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].z = 0;

	if (!(atomic64_read(&data->aSensorEnable) & (1 << SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD)))
		send_instruction(data, ADD_SENSOR, SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD,
			chTempbuf, 4);

	do {
		msleep(60);
		if (check_data_spec(data, SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD) == SUCCESS)
			break;
	} while (--retries);
	...
	iSensorBuf[0] = data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].x;
	iSensorBuf[1] = data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].y;
	iSensorBuf[2] = data->buf[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD].z;

	if (!(atomic64_read(&data->aSensorEnable) & (1 << SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD)))
		send_instruction(data, REMOVE_SENSOR, SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD,
			chTempbuf, 4);
...
{
	sensors_register(&data->devices[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD], data, mag_attrs,
		"magnetic_sensor");
}

void remove_magnetic_factorytest(struct ssp_data *data)
{
	sensors_unregister(data->devices[SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD], mag_attrs);
}
