	...
	unsigned int type, uint64_t new_enable)
{
	...
	if (type == SSP_SENSOR_TYPE_ORIENTATION) {
		if (!(atomic64_read(&data->aSensorEnable)
			& (1 << SSP_SENSOR_TYPE_ACCELEROMETER))) {
			type = SSP_SENSOR_TYPE_ACCELEROMETER;
		} else {
			change_sensor_delay(data, SSP_SENSOR_TYPE_ACCELEROMETER,
				data->delay[SSP_SENSOR_TYPE_ACCELEROMETER]);
			return 0;
		}
	} else if (type == SSP_SENSOR_TYPE_ACCELEROMETER) {
		if (atomic64_read(&data->aSensorEnable)
			& (1 << SSP_SENSOR_TYPE_ORIENTATION)) {
			change_sensor_delay(data, SSP_SENSOR_TYPE_ORIENTATION,
				data->delay[SSP_SENSOR_TYPE_ORIENTATION]);
			return 0;
		}
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	for (type = 0; type < SSP_SENSOR_TYPE_MAX; type++) {
		if ((atomic64_read(&data->aSensorEnable) & (1 << type))
			!= (new_enable & (1 << type))) {
			...
			if (!(new_enable & (1 << type))) {
				...
				if ((data->aiCheckStatus[type] == INITIALIZATION_STATE) &&
					(type == SSP_SENSOR_TYPE_ACCELEROMETER)) {
				...
				} else if ((data->aiCheckStatus[type] == INITIALIZATION_STATE) &&
					(type == SSP_SENSOR_TYPE_PRESSURE)) {
						pressure_open_calibration(data);
#endif
				} else if ((data->aiCheckStatus[type] == INITIALIZATION_STATE) &&
					(type == SSP_SENSOR_TYPE_PROXIMITY)) {
						set_proximity_threshold(data);
				}
...
static ssize_t show_acc_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_ACCELEROMETER);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((atomic64_read(&data->aSensorEnable) & (1 << SSP_SENSOR_TYPE_ORIENTATION)) &&
		(data->delay[SSP_SENSOR_TYPE_ORIENTATION] < delay))
		data->delay[SSP_SENSOR_TYPE_ACCELEROMETER] = delay;
	else
		change_sensor_delay(data, SSP_SENSOR_TYPE_ACCELEROMETER, delay);
...
static ssize_t show_gyro_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_GYROSCOPE);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_GYROSCOPE, delay);
	return size;
}
...
static ssize_t show_mag_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD, delay);
...
static ssize_t show_uncal_mag_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_MAGNETIC_FIELD_UNCALIBRATED);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_MAGNETIC_FIELD_UNCALIBRATED, delay);
...
static ssize_t show_rot_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_ROTATION_VECTOR);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_ROTATION_VECTOR, delay);
...
static ssize_t show_game_rot_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_GAME_ROTATION_VECTOR);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_GAME_ROTATION_VECTOR, delay);
...
static ssize_t show_step_det_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_STEP_DETECTOR);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_STEP_DETECTOR, delay);
	return size;
}
...
static ssize_t show_sig_motion_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_SIGNIFICANT_MOTION);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_SIGNIFICANT_MOTION, delay);
	return size;
}
...
static ssize_t show_step_cnt_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_STEP_COUNTER);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_STEP_COUNTER, delay);
	return size;
}
...
static ssize_t show_uncalib_gyro_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_GYROSCOPE_UNCALIBRATED);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_GYROSCOPE_UNCALIBRATED, delay);
...
static ssize_t show_pressure_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_PRESSURE);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_PRESSURE, delay);
	return size;
}
#endif
...
static ssize_t show_light_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_LIGHT);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_LIGHT, delay);
	return size;
}
...
static ssize_t show_prox_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_PROXIMITY);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_PROXIMITY, delay);
	return size;
}
...
static ssize_t show_tilt_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_TILT_DETECTOR);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_TILT_DETECTOR, delay);
	return size;
}
...
static ssize_t show_pickup_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return show_sensor_delay(dev, buf, SSP_SENSOR_TYPE_PICK_UP_GESTURE);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SSP_SENSOR_TYPE_PICK_UP_GESTURE, delay);
	return size;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d: %d\n%d: %d\n",
		SSP_SENSOR_TYPE_ACCELEROMETER, data->accel_position,
		SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD, data->mag_position);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = sscanf(buf, "%9d,%9d", &sensor, &position);

	if (position < 0 || position > 7)
		return -EINVAL;

	if (sensor == SSP_SENSOR_TYPE_ACCELEROMETER)
		data->accel_position = position;
	else if (sensor == SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD)
...
#if defined(CONFIG_SENSORS_SSP_STM32_MTK)
static ssize_t show_enable_sensor_data_dump(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", get_dump_sensor_data());
}

static ssize_t store_enable_sensor_data_dump(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int enable = 0;
	int ret = 0;
...
#if defined(CONFIG_SENSORS_SSP_STM32_MTK)
static DEVICE_ATTR(enable_sensor_data_dump, S_IRUGO | S_IWUSR | S_IWGRP,
	show_enable_sensor_data_dump, store_enable_sensor_data_dump);
#endif
...
#if defined(CONFIG_SENSORS_SSP_STM32_MTK)
	&dev_attr_enable_sensor_data_dump,
#endif
	NULL,
};
#if defined(CONFIG_SENSORS_SSP_STM32_MTK)
int ssp_mtk_sensors_enable(struct ssp_data *data, int sensor_type,
							int enable)
{
	int err = 0;
	size_t len = 0;
	...
	if (data->mcu_device == NULL) {
		pr_err("%s  data->mcu_device is NULL!\n", __func__);
		return -EINVAL;
	}
	new_enable = atomic64_read(&data->aSensorEnable);

	if (enable)
		new_enable |= 1 << sensor_type;
	else
		new_enable &= ~(1 << sensor_type);

	len = scnprintf(buf, sizeof(buf), "%llu", new_enable);
	err = set_sensors_enable(data->mcu_device, NULL, buf, len);

	if (err != len)
		return -EFAULT;
	return 0;
}
int ssp_mtk_sensors_set_delay(struct ssp_data *data, int sensor_type,
							uint64_t ms)
{
	int err = 0;
	size_t len = 0;
	...
	ssp_err("data=0x%p, data->mcu_device=0x%p\n", data, data->mcu_device);
	if (data->mcu_device == NULL) {
		pr_err("%s  data->mcu_device is NULL!\n", __func__);
		return -EINVAL;
	}

	len = scnprintf(buf, sizeof(buf), "%llu", ms);
	switch (sensor_type) {
	case SSP_SENSOR_TYPE_ACCELEROMETER:
		err = set_acc_delay(data->mcu_device, NULL, buf, len);
		break;
	case SSP_SENSOR_TYPE_GEOMAGNETIC_FIELD:
	case SSP_SENSOR_TYPE_ORIENTATION:
		err = set_mag_delay(data->mcu_device, NULL, buf, len);
		break;
	case SSP_SENSOR_TYPE_GYROSCOPE:
		err = set_gyro_delay(data->mcu_device, NULL, buf, len);
		break;
	case SSP_SENSOR_TYPE_LIGHT:
		err = set_light_delay(data->mcu_device, NULL, buf, len);
		break;
	case SSP_SENSOR_TYPE_PROXIMITY:
		err = set_prox_delay(data->mcu_device, NULL, buf, len);
		break;
	default:
		pr_err("%s sensor_type=%d is not support!\n", __func__, sensor_type);
		err = -EINVAL;
		break;
	}
	if (err != len)
				...
				unsigned long arg)
{
	...
	if (sensor_type >= SSP_SENSOR_TYPE_MAX) {
	...
	if (batch.timeout) { /* add or dry */
		...
		if (!(batch.flag & SSP_SENSORS_BATCH_DRY_RUN)) {
			ret = 1;
