...
void is_sec_copy_err_cnt_from_file(void)
{
	...
	pr_info("[BigData] %s REAR2 i2c_sensor(%d), mipi_sensor(%d)\n", __func__,
		cam_hwparam_collector.rear2_hwparam.i2c_sensor_err_cnt,
		cam_hwparam_collector.rear2_hwparam.mipi_sensor_err_cnt);
...
int is_sec_get_rear2_hw_param(struct cam_hw_param **hw_param)
{
...
// sysfs functions
#if !defined(titan6757_phone_n) && !defined(titan6757_jade_n)// J7Max and Jade are after PVR
#define SYSFS_BEFORE_PV
#endif

#if defined(titan6757_jade_n)
#define REAR_CAMFW_PREFIX "Z13KSKA"
#define REAR2_CAMFW_PREFIX "O05KSKA"
#define FRONT_CAMFW_PREFIX "O16KLJA"
#else // Default (J7Max)
...
#if defined(titan6757_jade_n)
extern void get_rear_tilt_cal_data(u8 *data);
extern void get_rear_mfr_info2(u8 *data);
...
{
	// TODO: check its value.
	char *camInfoStr = "ISP=INT;CALMEM=Y;READVER=SYSFS;COREVOLT=N;UPGRADE=N;FWWRITE=N;FWDUMP=N;CC=N;OIS=N;VALID=Y;";
	pr_err("%s : %s\n", __func__, camInfoStr);
...
static ssize_t rear_afcal_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 2+22+8+10, "10 %u %c %c %c %c %c %c %c %u\n", macro_cal, ch, ch, ch, ch, ch, ch, ch, pan_cal);
}
#else
//J7Max
static ssize_t rear_afcal_show(struct device *dev, struct device_attribute *attr, char *buf)
...
#if defined(titan6757_jade_n)
static ssize_t rear2_tilt_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int tiltdata[8], type=2;
	int *data_i = &tiltdata[0];	
	get_rear_tilt_cal_data((unsigned char*)&tiltdata[0]);		
	return snprintf(buf, 2+12*8+1, "%d %d %d %d %d %d %d %d %d\n", 
					type, data_i[0], data_i[1], data_i[2], data_i[3],data_i[4], data_i[5], data_i[6], data_i[7]);
}
static ssize_t rear2_afcal_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *localbuf = "NULL NULL NULL\n";
	pr_err("%s", __func__);	
	return snprintf(buf, sizeof(localbuf)+1,localbuf);
}
static ssize_t front_afcal_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char *localbuf = "NULL NULL NULL\n";
	 pr_err("%s", __func__); 
	 return snprintf(buf, sizeof(localbuf)+1,localbuf);
...
static ssize_t front2_afcal_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char *localbuf = "NULL NULL NULL\n";
	pr_err("%s", __func__);	
	return snprintf(buf, sizeof(localbuf)+1,localbuf);
...
static ssize_t rear2_camfw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char fw_version[12]={0};
	get_rear_mfr_info2(fw_version);
	pr_err("%s : fw ver2: %s\n", __func__, fw_version);
	return snprintf(buf, 24, "%s %s\n", fw_version, fw_version);
}

static ssize_t rear2_camfw_full_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char fw_version[12]={0};
	get_rear_mfr_info2(fw_version);
	pr_err("%s : fw ver2: %s\n", __func__, fw_version);
...
static ssize_t rear2_checkfw_factory_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char *retStr = "OK";
	char fw_version[12]={0};
	char *ref_version = REAR2_CAMFW_PREFIX;
	get_rear_mfr_info2(fw_version);
	if(!strncmp(fw_version, ref_version, strlen(ref_version))) {
#ifdef SYSFS_BEFORE_PV
		retStr = "OK";
...
static ssize_t rear2_checkfw_user_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char *retStr = "OK";
	char fw_version[12]={0};
	char *ref_version = REAR2_CAMFW_PREFIX;
...
static ssize_t rear2_camtype_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	// TODO: check its value.
	char *camTypeStr = "ISP\n";
	pr_err("%s : %s\n", __func__, camTypeStr);
...
{
	// TODO: check its value.
	char *camInfoStr = "ISP=INT;CALMEM=Y;READVER=SYSFS;COREVOLT=N;UPGRADE=N;FWWRITE=N;FWDUMP=N;CC=N;OIS=N;VALID=Y;DUALOPEN=N;";
	pr_err("%s : %s\n", __func__, camInfoStr);
	return snprintf(buf, 120, camInfoStr);
}

static ssize_t rear_dual_cal_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char rear_dual_cal[512+1]={0};
	unsigned int rear_dual_cal_size = get_rear_dual_cal_size();
	get_rear_dual_cal_data(rear_dual_cal);
	memcpy((void *)buf, (void *)rear_dual_cal, rear_dual_cal_size);
	pr_err("%s : dual cal data read %d bytes\n", __func__, rear_dual_cal_size);
	return rear_dual_cal_size;
}

static ssize_t rear_dual_cal_size_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int rear_dual_cal_size = get_rear_dual_cal_size();
	pr_err("%s : rear_dual_cal_size:%u\n", __func__, rear_dual_cal_size);
	return snprintf(buf, 6, "%u\n", rear_dual_cal_size);
}
#endif
...
#define FROM_MODULE_ID_SIZE	10
	...
	char rear_module_id[FROM_MODULE_ID_SIZE + 1] = "\0";
	pr_info("[Bigdata] %s\n", __func__);
	is_sec_get_rear2_hw_param(&ec_param);

	get_rear_moduleid(rear_module_id);

	return sprintf(buf, "\"CAMIR2_ID\":\"%c%c%c%c%cXX%02X%02X%02X\","
		"\"I2CR2_SEN\":\"%d\",\"MIPIR2_SEN\":\"%d\"\n",
		rear_module_id[0], rear_module_id[1], rear_module_id[2], rear_module_id[3],
		rear_module_id[4], rear_module_id[7], rear_module_id[8], rear_module_id[9],
		ec_param->i2c_sensor_err_cnt, ec_param->mipi_sensor_err_cnt);
}
...
static DEVICE_ATTR(rear2_hwparam, S_IRUGO|S_IWUSR|S_IWGRP, rear2_camera_hw_param_show, rear2_camera_hw_param_store);
#endif		/* CONFIG_CAMERA_HW_BIG_DATA */
...
static DEVICE_ATTR(rear2_camfw, S_IRUGO, rear2_camfw_show, NULL);
static DEVICE_ATTR(rear2_camfw_full, S_IRUGO, rear2_camfw_full_show, NULL);
static DEVICE_ATTR(rear2_checkfw_factory, S_IRUGO, rear2_checkfw_factory_show, NULL);
static DEVICE_ATTR(rear2_checkfw_user, S_IRUGO, rear2_checkfw_user_show, NULL);
static DEVICE_ATTR(rear2_camtype, S_IRUGO, rear2_camtype_show, NULL);
...
int create_rear_sysfs(void)
{
	...
	err = device_create_file(rear_dev, &dev_attr_rear2_afcal);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
				dev_attr_rear_afcal.attr.name);
	}

	err = device_create_file(rear_dev, &dev_attr_rear2_tilt);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
				dev_attr_rear_afcal.attr.name);
	}

	err = device_create_file(rear_dev, &dev_attr_rear2_camfw);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
				dev_attr_rear2_camfw.attr.name);
	}

	err = device_create_file(rear_dev, &dev_attr_rear2_camfw_full);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
				dev_attr_rear2_camfw_full.attr.name);
	}

	err = device_create_file(rear_dev, &dev_attr_rear2_checkfw_factory);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
				dev_attr_rear2_checkfw_factory.attr.name);
	}

	err = device_create_file(rear_dev, &dev_attr_rear2_checkfw_user);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
				dev_attr_rear2_checkfw_user.attr.name);
	}

	err = device_create_file(rear_dev, &dev_attr_rear2_camtype);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
	...
	err = device_create_file(rear_dev, &dev_attr_rear_dualcal);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
				dev_attr_rear_dualcal.attr.name);
	}

	err = device_create_file(rear_dev, &dev_attr_rear_dualcal_size);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
	...
	err = device_create_file(rear_dev, &dev_attr_rear2_hwparam);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
...
int create_front_sysfs(void)
{
	...
	err = device_create_file(front_dev, &dev_attr_front_afcal);
	if (unlikely(err < 0)) {
		pr_err("front_sysfs: failed to create device file, %s\n",
				dev_attr_front_camfw.attr.name);
	}

	err = device_create_file(front_dev, &dev_attr_front2_afcal);
	if (unlikely(err < 0)) {
		pr_err("front_sysfs: failed to create device file, %s\n",
				dev_attr_front_camfw.attr.name);
	}
...
bool _hwPowerOn(PowerType type, int powerVolt)
{
	...
	if (!IS_ERR(reg)) {
        ...
        if (type == SUB_DVDD && powerVolt == Vol_1050) {
			...
			if (type == DVDD)
				pmic_config_interface(PMIC_RG_VCAMD1_VOCAL_ADDR, 0x5, 0xf, 0);
				/*pmic_config_interface(0x1A74, 0x5, 0xf, 0); */
			else if (type == SUB_DVDD)
				pmic_config_interface(PMIC_RG_VCAMD2_VOCAL_ADDR, 0x5, 0xf, 0);
