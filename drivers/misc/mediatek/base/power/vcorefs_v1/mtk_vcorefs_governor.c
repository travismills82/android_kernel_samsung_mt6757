...
static struct vcorefs_info vcorefs_log;

void dump_vcorefs_log(void)
{
	pr_info("kicker: %d, opp: %d, vcore: %d(%d), fddr: %d(%d) %s%s, %d,%d, %d\n",
	vcorefs_log.kicker, vcorefs_log.dvfs_opp,
	vcorefs_log.vcore_uv, vcorefs_log.curr_vcore_uv,
	vcorefs_log.ddr_khz, vcorefs_log.curr_ddr_khz,
	vcorefs_log.vcore_dvs ? "[O]" : "[X]", vcorefs_log.ddr_dfs ? "[O]" : "[X]",
	vcorefs_log.expect_vcore_uv, vcorefs_log.expect_ddr_khz,
	vcorefs_log.kir_group);
}
EXPORT_SYMBOL(dump_vcorefs_log);
...
static int set_dvfs_with_opp(struct kicker_config *krconf)
{
	...
	vcorefs_log.kicker = krconf->kicker;
	vcorefs_log.dvfs_opp = krconf->dvfs_opp;

	vcorefs_log.vcore_uv = opp_ctrl_table[opp_idx].vcore_uv;
	vcorefs_log.ddr_khz = opp_ctrl_table[opp_idx].ddr_khz;

	vcorefs_log.curr_vcore_uv = gvrctrl->curr_vcore_uv;
	vcorefs_log.curr_ddr_khz = gvrctrl->curr_ddr_khz;

	vcorefs_log.vcore_dvs = gvrctrl->vcore_dvs;
	vcorefs_log.ddr_dfs = gvrctrl->ddr_dfs;

	vcorefs_log.expect_vcore_uv = expect_vcore_uv;
	vcorefs_log.expect_ddr_khz = expect_ddr_khz;

	vcorefs_log.kir_group = kir_group;
