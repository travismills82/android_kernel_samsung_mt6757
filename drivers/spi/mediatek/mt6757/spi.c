...
static int mt_spi_suspend(struct platform_device *pdev, pm_message_t message)
{
	...
	/*
	 * prevent suspend if spi still transfer.
	 * continue when spi transfer done.
	 */
	while (((ms->cur_transfer != NULL) || (ms->running != IDLE)) && limit--)
		usleep_range(10000, 11000);


	if ((ms->cur_transfer != NULL) || (ms->running != IDLE)) {
		SPI_ERR("spi%d could not stop over than 5sec!\n", pdev->id);
		return -EBUSY;
	}
