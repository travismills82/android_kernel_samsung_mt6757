...
/* SYSFS for service center */
static ssize_t sdcard_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{	
...
/* SYSFS for big data */
static ssize_t sdcard_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{	
	struct mmc_host *mmc = dev_get_drvdata(dev);
	struct mmc_card *card = mmc->card;
...
static DEVICE_ATTR(sd_count, S_IRUGO, sdcard_count_show, NULL);
static DEVICE_ATTR(sd_data, S_IRUGO, sdcard_data_show, NULL);
