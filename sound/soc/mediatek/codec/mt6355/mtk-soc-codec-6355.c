...
static const int dBFactor_Den = 32768;
...
void CalculateDCCompenForEachdB_L(void)
{
	...
	for (i = 0; i < MAX_HP_GAIN_LEVEL; i++) {  /* 0:8dB, 1:7dB, ...,  18:-10dB */
		mHplOffset_CompenBydB[i] = DIV_ROUND_CLOSEST((int64)mHplTrimOffset * dBFactor_Nom[i],
			dBFactor_Den);
...
void CalculateDCCompenForEachdB_R(void)
{
	...
	for (i = 0; i < MAX_HP_GAIN_LEVEL; i++) {  /* 0:8dB, 1:7dB, ...,  18:-10dB */
		mHprOffset_CompenBydB[i] = DIV_ROUND_CLOSEST((int64)mHprTrimOffset * dBFactor_Nom[i],
			dBFactor_Den);
...
static int calOffsetToDcComp(int TrimOffset)
{
	...
	tmp = TrimOffset;
	return DIV_ROUND_CLOSEST((int64)tmp * 2804225, 32768);
}
